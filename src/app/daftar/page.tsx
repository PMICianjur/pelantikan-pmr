...
export default function DaftarPage() {
  ...
  const kapasitasTendaTerpilih = useMemo(() => {
    ...
    if (sewaTenda === 'sendiri') return kapasitasTendaSendiri;
    return 0;
  ...
  const nextStep = () => {
    if (step === 1 && (!namaSekolah || !namaPembina || !nomorWhatsapp)) {
        setMessage('Harap lengkapi semua data sekolah dan pembina.'); setMessageType('error'); return;
    }
    if (step === 2) {
      if (pesertaList.length === 0) {
        setMessage('Anda belum mengunggah daftar peserta.'); setMessageType('error'); return;
      }
      if (stats.fotoSiap < stats.totalPeserta) {
        setMessage(`Masih ada ${stats.totalPeserta - stats.fotoSiap} foto peserta yang belum cocok.`); setMessageType('error'); return;
      }
    }
    if (step === 3) {
      if (!lahanDipilih) {
        setMessage('Anda harus memilih satu kavling di denah untuk melanjutkan.'); setMessageType('error'); return;
      }
    }
    setMessage('');
    setMessageType('info');
